...
<head>
    ...
    <meta name="keywords" content="论坛,社区,程序员">
    <title>Mybbs  </title>
...
<div class="panel panel-default" id="main" style="width: 55%;margin:1% 2% 1% 2%;float: left;">
    ...
    <li class="list-group-item">
        <c:if test="${topic.jinghua='1'}">【精华】</c:if>
            <a href="<%=basePath%>t/${topic.id}">${topic.title}</a>
            <c:if test="${user.type=='1'}">
            <div style="width: 10%;float: right;text-align: center;font-size: 12px;color: black">
                <a href="<%=basePath%>topic/jing/${topic.id}">帖子加精</a>
            </div>
            <div style="width: 10%;float: right;text-align: center;font-size: 12px;color: black">
                <a href="<%=basePath%>topic/quxiaojing/${topic.id}">取消加精</a>
            </div>
            </c:if><br/>
                <div>
